...
class AimlHandler(ContentHandler):
	...
	def _endElement(self, name):
		...
		if name == "aiml":
			...
			# 尝试将匹配项位置传出
			self.categories[key] = (self._elemStack[-1], self._position)
	...
	_validationInfo101 = {
		# 自定义标签
		"API":      (["type", "name"], ["key", "amount"], True),   # type属性表示调用的api类型 (目前想到的是 http 和 database )，name属性为请求的 api 名字
		#自定义标签，用于判定一段对话结束
		"end":([],[],True),
		# 原始标签
		"bot":      	( ["name"], [], False ),
