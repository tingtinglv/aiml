...
import string
import sys
from LanguageSupport import LangSupport
...
class PatternMgr:
	...
	def add(self, data, template, info=("filename", "line")):
		...
		# 添加文件信息
		node[self._TEMPLATE] = (template, info)
	...
	def match(self, pattern, that, topic):
		...
		# Language Support
		input_ = re.sub(self._upuncStripRE, " ", input_)
		input_ = self._langSupport(input_)
		...
		thatInput = re.sub(self._upuncStripRE, " ", thatInput)
		thatInput = self._langSupport(thatInput)
		...
		topicInput = re.sub(self._upuncStripRE, " ", topicInput)
		topicInput = self._langSupport(topicInput)
	...
	def star(self, starType, pattern, that, topic, index):
		...
		# Mutilate the input.  Remove all punctuation and convert the
		# text to all caps.
		pattern = self._langSupport(pattern)
		print("In star pattern : ",pattern)
		input_ = pattern.upper()
		input_ = re.sub(self._puncStripRE, " ", input_)
		input_ = re.sub(self._upuncStripRE, " ", input_)
	...
	def _match(self, words, thatWords, topicWords, root):
		...
		if len(words) == 0:
			...
			if template == None:
				...
				# 这是递归匹配的最后一项，也就是说 如果要加上那个查找的地址的话，应该在这里有记录
				pattern = []
		...
		if self._STAR in root:
			...
			for j in range(len(suffix)+1):
				...
				if template is not None:
					...
					# print("newPattern : {0}, template : {1}".format(newPattern,template))
					return (newPattern, template)
