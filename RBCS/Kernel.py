...
from APIProcessors import APIProcessors
...
class Kernel:
    # module constants
    _globalSessionID = AimlConfig.DEFAULT_ID # key of the global session (duh)
    ...
    _inputHistory = "inputHistory"     # keys to a queue (list) of recent user input
    _outputHistory = "outputHistory"   # keys to a queue (list) of recent responses.
    _inputStack = "inputStack"         # Should always be empty in between calls to respond()
    _collectionName = AimlConfig.varColl

    def __init__(self):
        self._verboseMode = True
        self._debug_inf = None
        self._aimlFile = ""
        ...
        # set up the API processors
        self._APIProcessors = APIProcessors()
        # set up mongoDB session
        self._SessionsDB = SessionDB(verbose=False)
        # set up the element processors
        self._elementProcessors = {
            # 自定义标签
            "API":          self._processAPI,
            "end":          self.__processEnd,
            # 原始标签
    ...
    def getPredicate(self, name, ownerID = _globalSessionID,userID = _globalSessionID, botID = _globalSessionID, validTime = AimlConfig.valitime,collectionName = _collectionName):
        ...
        try:
            return self._SessionsDB.getPredicate(name, self._aimlFile,ownerID,userID, botID, collectionName)
        except:
            return ""

    def setPredicate(self, name, value, ownerID = _globalSessionID,userID = _globalSessionID, botID = _globalSessionID,  validTime = AimlConfig.valitime,collectionName = _collectionName):
        ...
        # self._SessionsDB.addSession(sessionID)
        self._SessionsDB.setPredicate(name, value,self._aimlFile,ownerID, userID, botID, validTime,collectionName)
    ...
    def _deleteSession(self, sessionID):
        ...
        self._SessionsDB.deleteSession(sessionID)
    ...
    def getSessionData(self, sessionID = None):
        ...
        *all* of the individual session dictionaries.
        ...
        """
        s = self._SessionsDB.getSessionData(sessionID)
        if s is None:
            s = {}
    ...
    def learn(self, filename):
        ...
        if isinstance(filename, str):
            ...
            filename = map(lambda fl: fl.strip(), filename)
        ...
        for f in glob.glob(filename):
            ...
            for key, tem in handler.categories.items():
                ...
                # 添加文件信息
                self._brain.add(key, tem[0], (filename, tem[1]))
            # Parsing was successful.
            if self._verboseMode:
                print("done (%.2f seconds)" % (time.clock() - start))

    def respond(self, input_, ownerID=_globalSessionID, userID=_globalSessionID, botID=_globalSessionID, validTime = AimlConfig.valitime):
        ...
        try:
            ...
            # 获取history缓存在内存中
            self._SessionsDB.getHistoryFromDB(ownerID, userID, botID, limit=self._maxHistorySize)
            # split the input into discrete sentences
            sentences = Utils.sentences(input_)
            finalResponse = u""
            for s in sentences:
                """{"""
                # inputHistory = self.getPredicate(self._inputHistory, sessionID) # X
                inputHistory = self._SessionsDB.getHistory(self._inputHistory)
                inputHistory.append(s)
                while len(inputHistory) > self._maxHistorySize:
                    inputHistory.pop(0)
                # self.setPredicate(self._inputHistory, inputHistory, sessionID) # X
                self._SessionsDB.setInputHistory(inputHistory)
                """}"""
                # Fetch the response
                response = self._respond(s, ownerID, userID, botID, validTime)

                # add the data from this exchange to the history lists
                """{"""
                # outputHistory = self.getPredicate(self._outputHistory, sessionID)
                outputHistory = self._SessionsDB.getHistory(self._outputHistory)
                ...
                self._SessionsDB.setOutputHistory(outputHistory)
                """}"""
            ...
            assert(len(self._SessionsDB.getHistory(self._inputStack)) == 0)
            # and return, encoding the string into the I/O encoding
            return self._cod.enc(finalResponse)
        finally:
            # release the lock
            self._respondLock.release()
    def _respond(self, input_, ownerID, userID, botID, validTime):
        """Private version of respond(), does the real work."""
        if len(input_) == 0:
            return u""
        # guard against infinite recursion
        # inputStack = self.getPredicate(self._inputStack, sessionID)
        inputStack = self._SessionsDB.getHistory(self._inputStack)
        ...
        self._SessionsDB.setInputStack(inputStack)
        ...
        topic = self.getPredicate("topic", ownerID, userID, botID)
        ...
        if elem is None:
            ...
            response += self._processElement(elem, ownerID, userID, botID, validTime).strip()
            response += u" "
        response = response.strip()
        self._SessionsDB.setDialogState(ownerID, userID, botID, input_, response, filename=self._aimlFile,
                                        state=AimlConfig.dialog_state)
        AimlConfig.dialog_state = 0
    ...
    def _getResponse(self, result):
        if not self._debug_inf:
            return []
        response = {
            "txt": result,
            "path": self._debug_inf
        }
        return [response]
    def match(
            self, ownerid=_globalSessionID, userid=_globalSessionID, botid=_globalSessionID,
            timemark="NAN", moduletype=AimlConfig.moduletype, query="", validTime = AimlConfig.valitime):
        try:
            if moduletype != AimlConfig.moduletype:
                retV = Format.getFormat(
                    ownerid,userid,botid,timemark,moduletype,query,
                    [], AimlConfig.resp_type_error
                )
            else:
                self._debug_inf = None
                query=query.decode('utf-8')
                result = self.respond(query, ownerid, userid, botid, validTime)
                response_list = self._getResponse(result)
                response_list[0]["txt"]=response_list[0]["txt"].decode('utf-8')
                retV = Format.getFormat(
                    ownerid, userid, botid, timemark, moduletype, query,
                    response_list, AimlConfig.resp_success
                )
            return response_list[0]["txt"],retV
        except PyMongoTimeOutError:
            retV = Format.getFormat(
                ownerid, userid, botid, timemark, moduletype, query,
                [], AimlConfig.resp_time_out
            )
            return response_list[0]["txt"], retV
            ...
            retV = Format.getFormat(
                ownerid, userid, botid, timemark, moduletype, query,
                [], AimlConfig.resp_unknow_error, str(e)
            )
            return info,retV


    def _processElement(self, elem, ownerID, userID, botID, validTime):
        ...
        try:
            ...
            # 处理template文件信息
            handlerFunc = self._elementProcessors[elem[0][0]]
            # 输出匹配信息，用以debug
            self._debug_inf = \
                "Match  {0} :\n\tFileName : {1}\n\tLocation :{2}".format(elem[0][0], elem[1][0], elem[1][1])
            self._aimlFile = elem[1][0]
            return handlerFunc(elem[0], ownerID, userID, botID, validTime)
        except:
            try:
                handlerFunc = self._elementProcessors[elem[0]]
                return handlerFunc(elem, ownerID, userID, botID, validTime)
            except:
    ...
    # 自定义标签
    #<end>  作为结束标识
    def __processEnd(self,elem,ownerID,userID,botID,validTime):
        AimlConfig.dialog_state = 1
        topic = self.getPredicate("topic", ownerID, userID, botID)
        self.setPredicate("topic",topic , ownerID, userID, botID, validTime)
        AimlConfig.dialog_state = 0
        value = ""
        for e in elem[2:]:
            value += self._processElement(e, ownerID, userID, botID, validTime)
        return value
    # <API>
    def _processAPI(self, elem, ownerID, userID, botID, validTime):
        """处理 AIML标签 <API>
        Required element attributes:
            name: The name of the API
        调用名为 name 的API ，将API的内容作为参数传递过去，
        """
        APIType = elem[1][u'type']
        APIName = elem[1][u'name']
        APIExAttr = elem[1]
        APIReq = ""
        for e in elem[2:]:
            APIReq += self._processElement(e, ownerID, userID, botID, validTime)
        response = self._APIProcessors(APIType, APIName, APIExAttr, APIReq)
        return response
    # 原始标签
    # <bot>
    def _processBot(self, elem, ownerID, userID, botID, validTime):
    ...
    # <condition>
    def _processCondition(self, elem, ownerID, userID, botID,validTime):
        ...
        if 'name' in attr and 'value' in attr:
            val = self.getPredicate(attr['name'], ownerID, userID, botID)
            if val == attr['value']:
                for e in elem[2:]:
                    response += self._processElement(e, ownerID, userID, botID,validTime)
                return response
        else:
            ...
            try:
                ...
                for li in listitems:
                    try:
                        ...
                        # do the test
                        if self.getPredicate(liName, ownerID, userID, botID) == liValue:
                            foundMatch = True
                            response += self._processElement(li, ownerID, userID, botID, validTime)
                            break
                    except:
    ...
    # <date>
    def _processDate(self, elem, ownerID, userID, botID):
    ...
    # <formal>
    def _processFormal(self, elem, ownerID, userID, botID, validTime):
    ...
    # <gender>
    def _processGender(self, elem, ownerID, userID, botID,validTime):
    ...
    # <get>
    def _processGet(self, elem, ownerID, userID, botID,validTime = AimlConfig.valitime):
        ...
        """
        return self.getPredicate(elem[1]['name'], ownerID, userID, botID)

    # <gossip>
    def _processGossip(self, elem, ownerID, userID, botID,validTime):
        ...
        """        
        return self._processThink(elem, ownerID, userID, botID,validTime)

    # <id>
    def _processId(self, elem, ownerID, userID, botID,validTime):
        ...
        """        
        return ownerID + userID + botID

    # <input>
    def _processInput(self, elem, ownerID, userID, botID,validTime):
    ...
    # <javascript>
    def _processJavascript(self, elem, ownerID, userID, botID,validTime):
    ...
    # <learn>
    def _processLearn(self, elem, ownerID, userID, botID,validTime):
        ...
        filename = ""
        for e in elem[2:]:
            filename += self._processElement(e, ownerID, userID, botID,validTime)
        self.learn(filename)
        return ""

    # <li>
    def _processLi(self, elem, ownerID, userID, botID,validTime):
    ...
    # <lowercase>
    def _processLowercase(self, elem, ownerID, userID, botID,validTime):
    ...
    # <person>
    def _processPerson(self, elem, ownerID, userID, botID,validTime):
        ...
        if len(elem[2:]) == 0:  # atomic <person/> = <person><star/></person>
            response = self._processElement(['star',{}], ownerID, userID, botID,validTime)
        return self._subbers['person'].sub(response)

    # <person2>
    def _processPerson2(self, elem, ownerID, userID, botID,validTime):
    ...
    # <random>
    def _processRandom(self, elem, ownerID, userID, botID,validTime):
        ...
        return self._processElement(listitems[0], ownerID, userID, botID,validTime)
        
    # <sentence>
    def _processSentence(self, elem, ownerID, userID, botID,validTime):
    ...
    # <set>
    def _processSet(self, elem, ownerID, userID, botID,validTime):
        ...
        for e in elem[2:]:
            value += self._processElement(e, ownerID, userID, botID,validTime)
        self.setPredicate(elem[1]['name'], value, ownerID, userID, botID,validTime)
        return value

    # <size>
    def _processSize(self, elem, ownerID, userID, botID, validTime):
    ...
    # <sr>
    def _processSr(self, elem, ownerID, userID, botID,validTime):
        """Process an <sr> AIML element.

        <sr> elements are shortcuts for <srai><star/></srai>.

        """
        star = self._processElement(['star',{}], ownerID, userID, botID,validTime)
        response = self._respond(star, ownerID, userID, botID,validTime)
        return response

    # <srai>
    def _processSrai(self, elem, ownerID, userID, botID,validTime):
        ...
        newInput = ""
        for e in elem[2:]:
            newInput += self._processElement(e, ownerID, userID, botID,validTime)
        return self._respond(newInput, ownerID, userID, botID,validTime)

    # <star>
    def _processStar(self, elem, ownerID, userID, botID,validTime):
    ...
    # <system>
    def _processSystem(self, elem, ownerID, userID, botID,validTime):
        ...
        command = ""
        for e in elem[2:]:
            command += self._processElement(e, ownerID, userID, botID,validTime)
    ...
    # <template>
    def _processTemplate(self, elem, ownerID, userID, botID,validTime):
    ...
    # text
    def _processText(self, elem, ownerID, userID, botID, validTime):
    ...
    # <that>
    def _processThat(self, elem, ownerID, userID, botID, validTime):
    ...
    # <thatstar>
    def _processThatstar(self, elem, ownerID, userID, botID, validTime):
    ...
    # <think>
    def _processThink(self,elem, ownerID, userID, botID, validTime):
        ...
        """
        for e in elem[2:]:
            self._processElement(e, ownerID, userID, botID, validTime)
        return ""

    # <topicstar>
    def _processTopicstar(self, elem, ownerID, userID, botID, validTime):
    ...
    # <uppercase>
    def _processUppercase(self, elem, ownerID, userID, botID, validTime):
    ...
    # <version>
    def _processVersion(self, elem, ownerID, userID, botID, validTime):
